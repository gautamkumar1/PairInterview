                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
                        ...
                        <Card className="bg-card border-border shadow-sm">
                            ...
                            <CardContent>
                                <div className="flex items-center justify-between">
                                    <div className="text-5xl font-bold text-foreground font-mono">{activeSessionCount}</div>
                                    <div className="flex items-center gap-2">
                                        <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold bg-primary/15 text-green-500 ring-1 ring-primary/30">
                                            <span className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></span>
                                            LIVE
                                        </span>
                                    </div>
                                </div>
                                <p className="text-xs text-muted-foreground mt-2">Real-time 1v1 battles</p>
                            </CardContent>
                        </Card>
                        ...
                        <Card className="bg-card border-border shadow-sm">
                            ...
                            <CardContent>
                                ...
                                <p className="text-xs text-muted-foreground mt-2">All-time practice & battles</p>
                            </CardContent>
                        </Card>
                    </div>

                    {/* Full-width Live Sessions Card */}
                    <div className="mt-2">
                        <Card className="bg-card border-border shadow-sm">
                            <CardHeader className="pb-3">
                                <CardTitle className="text-sm font-medium text-muted-foreground tracking-wider flex items-center gap-2">
                                ...
                                <div className="space-y-3 max-h-64 overflow-y-auto">
                                    {liveSessions.length > 0 ? (
                                        liveSessions.map((session) => (
                                            <div
                                                key={session.id}
                                                className="p-4 bg-muted rounded-lg border border-border hover:bg-accent/50 transition-all duration-200 cursor-pointer"
                                            >
                                                <div className="flex items-start justify-between mb-2">
                                                    <div>
                                                        <div className="font-semibold text-foreground">{session.problemName}</div>
                                                        <div className="text-xs text-muted-foreground mt-1">
                                                            by <span className="font-medium">{session.creatorName}</span> • {getTimeAgo(session.createdAt)}
                                                        </div>
                                                    </div>
                                                    <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold ${getDifficultyColor(session.difficulty)}`}>
                                                        {session.difficulty}
                                                    </span>
                                                </div>
                                                ...
                                                <div className="flex items-center justify-between text-xs">
                                                    ...
                                                    <div className="flex gap-2">
                                                        {session.participants < session.maxParticipants ? (
                                                            ...
                                                            <Button disabled size="sm" variant="secondary" className="h-8 text-xs">
                                                                Full
                                                            </Button>
                                                        )}
                                        ...
                                        <p className="text-center text-muted-foreground py-8 text-sm">
                                            No live sessions right now. Create one!
                                        </p>
                                    )}
                                </div>
                            </CardContent>
